...
export const PokemonDetailScreen = () => {
  ...
  const targetEntries = useMemo(() => {
    if (!species) return [];
    const ja = species.flavor_text_entries.filter(e => e.language.name === 'ja' || e.language.name === 'ja-Hrkt');
  ...
  if (!pokemon || !species) {
    return (
      <div className="text-center py-20">
        <p className="text-red-500 text-xl font-bold mb-4">Pokemon not found!</p>
        <button onClick={() => navigate(-1)} className="px-6 py-3 bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-white rounded-xl shadow-sm hover:shadow-md transition-all font-bold">Back to Pokédex</button>
      </div>
    );
  }
  ...
  const genus = jaGenus?.genus || enGenus?.genus || '';
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[2rem] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700/60">
        ...
        <div className="p-8 md:p-16 border-b border-slate-100 dark:border-slate-700/60 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800/80 dark:to-slate-800 flex flex-col md:flex-row items-center gap-12 relative overflow-hidden">
          ...
          <div className="w-72 h-72 md:w-96 md:h-96 relative z-10 drop-shadow-2xl hover:scale-105 transition-transform duration-500">
             {imageUrl ? <img src={imageUrl} alt={pokemon.name} className="w-full h-full object-contain filter" /> : <div className="w-full h-full bg-slate-200 rounded-full flex items-center justify-center font-bold text-slate-400">No Image</div>}
          </div>

          <div className="flex-1 z-10 w-full">
            <div className="flex flex-col gap-2 mb-4">
              <span className="text-2xl font-black text-slate-400/80 tracking-widest">#{String(pokemon.id).padStart(4, '0')}</span>
              <h2 className="text-5xl md:text-6xl font-black text-slate-800 dark:text-white capitalize tracking-tighter drop-shadow-sm">{pokemon.name}</h2>
            </div>
            ...
            <div className="flex gap-3 mb-10">
              {pokemon.types.map((t) => (
                <span key={t.type.name} className={`px-6 py-2.5 rounded-full text-sm font-black uppercase tracking-widest shadow-md ${getTypeColor(t.type.name)}`}>
                  {t.type.name}
                </span>
              ))}
            </div>
            ...
            <div className="bg-white/60 dark:bg-slate-900/60 p-6 md:p-8 rounded-3xl border border-white/40 dark:border-slate-700/40 shadow-inner backdrop-blur-sm -skew-x-1">
              <div className="flex justify-between items-center mb-4 border-b border-slate-300/50 dark:border-slate-600/50 pb-3 h-[2rem]">
                <span className="font-bold text-slate-500 dark:text-slate-400 text-sm tracking-widest uppercase">Pokédex Entry</span>
                <select 
                ...
                >
                  {availableVersions.map(v => (
                    <option key={v} value={v} className="bg-white dark:bg-slate-800">{v.replace('-', ' ')}</option>
                  ))}
                </select>
              </div>
        ...
        <div className="p-8 md:p-16 grid grid-cols-1 lg:grid-cols-2 gap-12 bg-white dark:bg-slate-800">
          ...
          <div className="space-y-12">
            <div>
              <h3 className="text-2xl font-black mb-8 text-slate-800 dark:text-white border-b-2 border-slate-100 dark:border-slate-700 pb-4 inline-block">Physical Traits</h3>
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700/50 text-center">
                  <p className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Height</p>
                ...
                <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700/50 text-center">
                  <p className="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Weight</p>
          ...
          <div>
             <h3 className="text-2xl font-black mb-8 text-slate-800 dark:text-white border-b-2 border-slate-100 dark:border-slate-700 pb-4 inline-block">Base Stats</h3>
             <div className="bg-slate-50 dark:bg-slate-900 rounded-3xl p-8 shadow-sm border border-slate-100 dark:border-slate-700/50 space-y-6">
               {pokemon.stats.map(s => {
                  ...
                  return (
                    <div key={s.stat.name} className="flex items-center gap-6">
                      <span className="w-32 text-sm font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">{s.stat.name.replace('-', ' ')}</span>
               ...
               <div className="flex items-center gap-6 pt-4 border-t border-slate-200 dark:border-slate-700">
                 <span className="w-32 text-sm font-black text-slate-600 dark:text-slate-300 uppercase tracking-widest">Total</span>
