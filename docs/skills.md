...
## 🛣 TanStack Router & ディレクトリ設計
 
### コロケーション（Colocation）戦略
TanStack Router の「ファイルベースルーティング」と「画面専用コンポーネント」を共存させるためのテクニック：
 
- **プレフィックスによる無視**: フォルダ名やファイル名を `-` (ハイフン) または `_` (アンダースコア) で始めることで、ルーターの自動生成対象から除外できる。
- **ルートレベル・コロケーション**:
  - `/pokemon/index.tsx` (一覧画面) ➔ `./-index/components/` に部品を配置。
  - `/pokemon/$name.tsx` (詳細画面) ➔ `./-$name/components/` に部品を配置。
- **メリット**: 
  - エディタ上で画面ファイルと部品フォルダが隣接するため、開発効率が最大化される。
  - グローバルな `src/components` が肥大化するのを防ぎ、依存関係を局所化できる。
  - `$name` のように動的パラメータ名を含めることで、どのルートに対応するフォルダかが一目でわかる。
...
- **型推論**: `z.infer<typeof schema>` を使用して、コンポーネント側でも型安全にパラメータを参照する。
- **navigate での更新**: `navigate({ search: (prev) => ({ ...prev, type: 'fire' }) })` のように関数形式で更新することで、他の既存の検索パラメータを維持したまま特定の値を変更できる。
- **IME（日本語入力）対策**: `onChange` で即座に `navigate` を実行すると、再レンダリングによりIMEの変換状態がリセットされる。これを防ぐには、入力値を一時的にローカルな `useState` で保持し、入力停止から 300ms 程度待ってから `navigate` を実行する「デバウンス処理」が必須。
 
### 🔄 デバウンス同期パターン (`useDebouncedSync`)
URL Search Params や Props とローカルの `input` 状態を同期させる際のベストプラクティス：
- **課題**: `onChange` で即座に外部状態（URLなど）を更新すると、再レンダリングによりIMEの変換が壊れたり、APIリクエストが過剰に発生したりする。
- **解決策**: ローカル状態（即時反映）と外部同期（デバウンス反映）の二段構えにする。
- **実装方法**: `useDebouncedSync` フック。
  1. `externalValue`（URL等）が変わったら `localValue` を即座に同期（`useEffect`）。
  2. `localValue`（入力）が変わったら 300ms 待ってから `onDebouncedChange`（URL更新等）を実行（`useEffect` + `setTimeout`）。
- **メリット**: UX（キビキビした入力感）とパフォーマンス（負荷軽減）の両立が、呼び出し側わずか1行で実現できる。
...
- **開発**: `BASE_URL = 'https://pokeapi.co/api/v2'` → PokeAPI 直叩き。
